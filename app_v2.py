...
# === TABS ===
tab1, tab2, tab3 = st.tabs(["\U0001f4ca Weekly Report", "\U0001f4dd Quick Summary", "\U0001f4c8 Q Report"])
        ...
        if st.button("\U0001f680 Generate Summary", type="primary", use_container_width=True, key="t3_gen"):
            ...
            with st.spinner("Generating..."):
                try:
                    ...
                    # Use PowerPoint COM to export as PNG
                    png_bytes = None
                    try:
                        import comtypes.client
                        tmp_pptx = os.path.join(tempfile.gettempdir(), "summary_export.pptx")
                        tmp_png_dir = os.path.join(tempfile.gettempdir(), "summary_png_out")
...
with tab3:
    st.subheader("\U0001f4c8 Quarterly Report Generator")
    st.markdown("Generate an executive-level quarterly performance report (portrait PPTX, 2 slides).")
    ...
    with col_q1:
        q_label = st.text_input("Quarter Label", value="Q2 2026", key="q_label")
    with col_q2:
        q_period = st.text_input("Period (e.g. 'April – June')", value="April – June", key="q_period")
    ...
    with kpi_c1:
        q_games = st.number_input("Games Released", value=4768, min_value=0, key="q_games")
        q_games_note = st.text_input("Games subtitle", value="18 markets • 11 weeks", key="q_games_note")
    with kpi_c2:
        q_containers = st.number_input("Containers Launched", value=72, min_value=0, key="q_containers")
        q_containers_note = st.text_input("Containers subtitle", value="Incl. World Cup 2026 campaign", key="q_cont_note")
    with kpi_c3:
        q_features = st.number_input("Features Enabled", value=4, min_value=0, key="q_features")
        q_features_note = st.text_input("Features subtitle", value="3D Carousel • Slot Races • More", key="q_feat_note")

    st.divider()

    # --- Strategic Highlights ---
    st.subheader("Strategic Highlights (up to 3)")
    sh_titles = []
    sh_bodies = []
    for i in range(3):
        with st.expander(f"Highlight {i+1}", expanded=(i == 0)):
            t = st.text_input(f"Title {i+1}", key=f"q_sh_title_{i}",
                              value=["World Cup 2026 Activation", "Live Casino Transformation", "Platform & UX Innovation"][i] if i < 3 else "")
            b = st.text_area(f"Description {i+1}", key=f"q_sh_body_{i}", height=80,
                             value=[
                                 "Multi-market promotional deployment across UK, EU, Italy, Belgium, Greece & Spain. Coordinated launches of Cup Fever, KICK&SPIN, Spin Like A World Champion, and Goal Zone containers driving cross-brand engagement.",
                                 "Complete Gala Casino live casino restructure delivered — 14 new containers covering Exclusives, Roulette, Blackjack, Game Shows, Baccarat, Poker and provider categories. Sports Arena containers launched for Sportingbet.uk and Bwin.uk.",
                                 "3D Carousel Tile shipped to 4 brands. New Top Navigation for Sportingbet.uk. Slot Races feature enabled for Brazil. Game Premier rolled out across UK Gaming brands.",
                             ][i] if i < 3 else "")
            sh_titles.append(t)
            sh_bodies.append(b)
    ...
    # --- Launch Type Comparison (Q-over-Q chart) ---
    st.subheader("Launch Type Comparison (Q-over-Q)")
    st.caption("Enter launch categories and counts for previous vs current quarter.")

    default_launch_data = [
        ("Market Release", 2119, 3643),
        ("Early Release", 272, 586),
        ("Market Catchup", 1108, 1279),
        ("Exclusive", 63, 122),
        ("Gap Close", 16, 206),
        ("Relocated Rel.", 100, 63),
        ("Specific Mkt Rel.", 230, 386),
        ("Game Switch", 507, 345),
        ("Top 300 Catch Up", 91, 70),
        ("New Provider RO", 367, 3696),
    ]

    q_prev_label = st.text_input("Previous quarter label", value="Q1'26", key="q_prev_lbl")
    q_curr_label = st.text_input("Current quarter label", value="Q2'26", key="q_curr_lbl")

    launch_text = st.text_area(
        "Launch data (one per line: Category, PrevQ, CurrQ)",
        value="\n".join(f"{cat}, {p}, {c}" for cat, p, c in default_launch_data),
        height=220, key="q_launch_data"
    )

    st.divider()

    # --- Slide 2: Top Markets ---
    st.subheader("Top Markets by Volume (Slide 2)")
    top_markets_text = st.text_area(
        "Top markets (one per line: Market, Count)",
        value=".com (EU), 476\nAustria, 448\nBelgium, 401\nOntario, 338\nUK (Sports), 338\nGreece, 313",
        height=140, key="q_top_markets"
    )

    st.divider()

    # --- Game Quality Distribution ---
    st.subheader("Game Quality Distribution")
    quality_text = st.text_area(
        "Ratings (one per line: Rating, Count)",
        value="Platinum, 1073\nGold, 4826\nSilver, 4424\nGOTM, 30\nExclusive, 122",
        height=120, key="q_quality"
    )

    st.divider()

    # --- Key Initiatives ---
    st.subheader("Key Initiatives Delivered")
    initiatives_text = st.text_area(
        "Initiatives (one per line, use → to separate feature from brands)",
        value="3D Carousel Tile → SICOM, SOCOM, Bwin.es, Party.es\nNew Top Navigation → Sportingbet.uk\nSlot Races → Sportingbet.bet.br\nGame Premier → Gala Casino, Gala Bingo, Foxy Bingo\nWorld Cup 2026 campaign → 12+ brands across 6 markets\nLive Casino redesign → Gala Casino (14 containers)",
        height=140, key="q_initiatives"
    )

    st.divider()

    # --- Generate ---
    if st.button("\U0001f680 Generate Q Report", type="primary", use_container_width=True, key="q_gen"):
        with st.spinner("Building quarterly report..."):
            try:
                # Parse launch data
                launch_cmp_parsed = []
                for line in launch_text.strip().split("\n"):
                    parts = [p.strip() for p in line.split(",")]
                    if len(parts) >= 3:
                        launch_cmp_parsed.append((parts[0], int(parts[1]), int(parts[2])))

                # Parse top markets
                top_markets_parsed = []
                for line in top_markets_text.strip().split("\n"):
                    parts = [p.strip() for p in line.split(",")]
                    if len(parts) >= 2:
                        top_markets_parsed.append((parts[0], parts[1]))

                # Parse quality distribution
                quality_parsed = []
                quality_colors = [
                    RGBColor(0x9C, 0x27, 0xB0),  # Platinum - Purple
                    RGBColor(0xF9, 0xA8, 0x25),  # Gold
                    RGBColor(0x78, 0x90, 0x9C),  # Silver
                    RGBColor(0x66, 0xBB, 0x6A),  # GOTM
                    RGBColor(0x26, 0xA6, 0x9A),  # Exclusive
                ]
                for idx_q, line in enumerate(quality_text.strip().split("\n")):
                    parts = [p.strip() for p in line.split(",")]
                    if len(parts) >= 2:
                        clr = quality_colors[idx_q] if idx_q < len(quality_colors) else RGBColor(0x60, 0x60, 0x60)
                        quality_parsed.append((parts[0], f"{int(parts[1]):,}", clr))

                # Parse initiatives
                initiatives_parsed = [l.strip() for l in initiatives_text.strip().split("\n") if l.strip()]

                # Build strategic highlights
                highlights_parsed = [(t, b) for t, b in zip(sh_titles, sh_bodies) if t.strip()]

                # === Generate PPTX ===
                prs_q = Presentation()
                prs_q.slide_width = Inches(7.5)
                prs_q.slide_height = Inches(10)

                # Colors
                Q_PURPLE = RGBColor(0x4A, 0x00, 0x72)
                Q_PURPLE_L = RGBColor(0x7B, 0x1F, 0xA2)
                Q_MAGENTA = RGBColor(0xAB, 0x28, 0x7E)
                Q_TEAL = RGBColor(0x00, 0x96, 0x88)
                Q_WHITE = RGBColor(0xFF, 0xFF, 0xFF)
                Q_BLACK = RGBColor(0x21, 0x21, 0x21)
                Q_GRAY = RGBColor(0x61, 0x61, 0x61)
                Q_LIGHT_GRAY = RGBColor(0xF5, 0xF5, 0xF5)
                Q_BAR_Q1 = RGBColor(0xCE, 0x93, 0xD8)
                Q_BAR_Q2 = RGBColor(0x6A, 0x1B, 0x9A)

                def q_rect(sl, l, t, w, h, color):
                    s = sl.shapes.add_shape(MSO_SHAPE.RECTANGLE, l, t, w, h)
                    s.fill.solid(); s.fill.fore_color.rgb = color; s.line.fill.background()
                    return s

                def q_rrect(sl, l, t, w, h, color):
                    s = sl.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, l, t, w, h)
                    s.fill.solid(); s.fill.fore_color.rgb = color; s.line.fill.background()
                    return s

                def q_txt(sl, l, t, w, h, text, sz=Pt(9), bold=False, color=Q_BLACK,
                          align=PP_ALIGN.LEFT, va=MSO_ANCHOR.MIDDLE):
                    ...
                    tf.margin_top = Emu(9144); tf.margin_bottom = Emu(9144)
                    p = tf.paragraphs[0]; p.alignment = align
                    r = p.add_run(); r.text = text; r.font.name = 'Mulish'
                    r.font.size = sz; r.font.bold = bold; r.font.color.rgb = color
                    return b

                def q_kpi_block(sl, x, y, w, h, value, label, sublabel, accent):
                    q_rrect(sl, x, y, w, h, Q_LIGHT_GRAY)
                    q_rect(sl, x, y + Emu(45720), Emu(36576), h - Emu(91440), accent)
                    q_txt(sl, x + Emu(54864), y + Emu(27432), w - Emu(91440), Emu(411480),
                          value, sz=Pt(30), bold=True, color=accent, align=PP_ALIGN.CENTER)
                    q_txt(sl, x + Emu(54864), y + Emu(411480), w - Emu(91440), Emu(201168),
                          label, sz=Pt(9), bold=True, color=Q_BLACK, align=PP_ALIGN.CENTER)
                    q_txt(sl, x + Emu(54864), y + Emu(594360), w - Emu(91440), Emu(164592),
                          sublabel, sz=Pt(7), color=Q_GRAY, align=PP_ALIGN.CENTER)

                # ===== SLIDE 1: Executive Dashboard =====
                s1 = prs_q.slides.add_slide(prs_q.slide_layouts[6])
                SW = prs_q.slide_width
                SH = prs_q.slide_height
                M = Emu(228600)
                y = 0

                # Header
                q_rect(s1, 0, y, SW, Emu(457200), Q_PURPLE)
                q_txt(s1, 0, y + Emu(45720), SW, Emu(274320),
                      "Lobby Operations", sz=Pt(18), bold=True, color=Q_WHITE, align=PP_ALIGN.CENTER)
                q_txt(s1, 0, y + Emu(310896), SW, Emu(164592),
                      f"Quarterly Performance Report  |  {q_label} ({q_period})",
                      sz=Pt(9), color=RGBColor(0xCE, 0x93, 0xD8), align=PP_ALIGN.CENTER)
                y += Emu(548640)

                # KPI row
                kpi_w = Emu(2011680); kpi_h = Emu(777240)
                kpi_gap = (SW - 3 * kpi_w) // 4
                q_kpi_block(s1, kpi_gap, y, kpi_w, kpi_h,
                            f"{q_games:,}", "Games Released", q_games_note, Q_PURPLE)
                q_kpi_block(s1, kpi_gap * 2 + kpi_w, y, kpi_w, kpi_h,
                            f"{q_containers}+", "Containers Launched", q_containers_note, Q_MAGENTA)
                q_kpi_block(s1, kpi_gap * 3 + kpi_w * 2, y, kpi_w, kpi_h,
                            str(q_features), "Features Enabled", q_features_note, Q_TEAL)
                y += kpi_h + Emu(228600)

                # Strategic Highlights
                q_txt(s1, M, y, SW - 2 * M, Emu(228600),
                      "Strategic Highlights", sz=Pt(12), bold=True, color=Q_PURPLE)
                q_rect(s1, M, y + Emu(228600), Emu(2743200), Emu(18288), Q_PURPLE)
                y += Emu(292608)

                for i, (htitle, hbody) in enumerate(highlights_parsed):
                    bx = M + Emu(45720)
                    q_rrect(s1, M, y + Emu(9144), Emu(228600), Emu(228600), Q_PURPLE)
                    q_txt(s1, M, y + Emu(9144), Emu(228600), Emu(228600),
                          str(i + 1), sz=Pt(11), bold=True, color=Q_WHITE, align=PP_ALIGN.CENTER)
                    q_txt(s1, bx + Emu(228600), y, SW - bx - Emu(274320), Emu(228600),
                          htitle, sz=Pt(10), bold=True, color=Q_PURPLE_L)
                    q_txt(s1, bx + Emu(228600), y + Emu(210312), SW - bx - Emu(274320), Emu(365760),
                          hbody, sz=Pt(7), color=Q_GRAY, va=MSO_ANCHOR.TOP)
                    y += Emu(594360)

                # Chart: Q-over-Q comparison
                y += Emu(91440)
                q_txt(s1, M, y, SW - 2 * M, Emu(228600),
                      "Quarter-over-Quarter: Launch Type Comparison", sz=Pt(11), bold=True, color=Q_PURPLE)
                q_rect(s1, M, y + Emu(228600), Emu(2743200), Emu(18288), Q_PURPLE)
                y += Emu(292608)

                chart_h = SH - y - Emu(182880)
                chart_w = SW - 2 * M

                chart_data = CategoryChartData()
                chart_data.categories = [c[0] for c in launch_cmp_parsed]
                chart_data.add_series(q_prev_label, [c[1] for c in launch_cmp_parsed])
                chart_data.add_series(q_curr_label, [c[2] for c in launch_cmp_parsed])

                cf = s1.shapes.add_chart(XL_CHART_TYPE.COLUMN_CLUSTERED, M, y, chart_w, chart_h, chart_data)
                chart = cf.chart
                chart.has_legend = True
                chart.legend.position = XL_LEGEND_POSITION.BOTTOM
                chart.legend.include_in_layout = False
                chart.legend.font.size = Pt(8)
                chart.legend.font.color.rgb = Q_GRAY
                plot = chart.plots[0]
                plot.gap_width = 100
                plot.series[0].format.fill.solid()
                plot.series[0].format.fill.fore_color.rgb = Q_BAR_Q1
                plot.series[1].format.fill.solid()
                plot.series[1].format.fill.fore_color.rgb = Q_BAR_Q2
                chart.value_axis.has_major_gridlines = True
                chart.value_axis.major_gridlines.format.line.color.rgb = RGBColor(0xEE, 0xEE, 0xEE)
                chart.value_axis.tick_labels.font.size = Pt(7)
                chart.value_axis.tick_labels.font.color.rgb = Q_GRAY
                chart.category_axis.tick_labels.font.size = Pt(7)
                chart.category_axis.tick_labels.font.color.rgb = Q_GRAY

                # ===== SLIDE 2: Delivery & Outlook =====
                s2 = prs_q.slides.add_slide(prs_q.slide_layouts[6])
                y = 0

                # Header
                q_rect(s2, 0, y, SW, Emu(365760), Q_PURPLE)
                q_txt(s2, 0, y, SW, Emu(365760),
                      "Delivery Breakdown & Market Coverage", sz=Pt(14), bold=True,
                      color=Q_WHITE, align=PP_ALIGN.CENTER)
                y += Emu(457200)

                # Top Markets
                q_txt(s2, M, y, SW - 2 * M, Emu(228600),
                      "Top Markets by Volume", sz=Pt(11), bold=True, color=Q_PURPLE)
                q_rect(s2, M, y + Emu(228600), Emu(1828800), Emu(18288), Q_PURPLE)
                y += Emu(320040)

                market_colors_q = [
                    RGBColor(0x6E, 0xE7, 0xB7), RGBColor(0xFD, 0xE6, 0x8A),
                    RGBColor(0xFC, 0xA5, 0xA5), RGBColor(0xC4, 0xB5, 0xFD),
                    RGBColor(0x67, 0xE8, 0xF9), RGBColor(0xF9, 0xA8, 0xD4),
                ]
                num_mkt = min(len(top_markets_parsed), 6)
                if num_mkt > 0:
                    card_w_q = Emu(1005840); card_h_q = Emu(548640)
                    card_gap_q = (SW - 2 * M - num_mkt * card_w_q) // max(num_mkt - 1, 1)
                    for i, (mkt, val) in enumerate(top_markets_parsed[:6]):
                        x = M + i * (card_w_q + card_gap_q)
                        clr = market_colors_q[i % len(market_colors_q)]
                        q_rrect(s2, x, y, card_w_q, card_h_q, clr)
                        q_txt(s2, x, y + Emu(91440), card_w_q, Emu(274320),
                              val, sz=Pt(20), bold=True, color=Q_BLACK, align=PP_ALIGN.CENTER)
                        q_txt(s2, x, y + Emu(365760), card_w_q, Emu(164592),
                              mkt, sz=Pt(7), bold=True, color=RGBColor(0x33, 0x33, 0x33), align=PP_ALIGN.CENTER)
                    y += card_h_q + Emu(274320)

                # Game Quality Distribution
                q_txt(s2, M, y, SW - 2 * M, Emu(228600),
                      "Game Quality Distribution", sz=Pt(11), bold=True, color=Q_PURPLE)
                q_rect(s2, M, y + Emu(228600), Emu(1828800), Emu(18288), Q_PURPLE)
                y += Emu(320040)

                bar_w_q = SW - 2 * M
                bar_h_q = Emu(320040)
                for i, (name, val, col) in enumerate(quality_parsed):
                    by = y + i * (bar_h_q + Emu(45720))
                    q_rrect(s2, M, by, bar_w_q, bar_h_q, Q_LIGHT_GRAY)
                    q_rect(s2, M, by + Emu(27432), Emu(45720), bar_h_q - Emu(54864), col)
                    q_txt(s2, M + Emu(91440), by, Emu(1371600), bar_h_q,
                          name, sz=Pt(10), bold=True, color=Q_BLACK)
                    q_txt(s2, bar_w_q - Emu(914400), by, Emu(1143000), bar_h_q,
                          val, sz=Pt(14), bold=True, color=col, align=PP_ALIGN.RIGHT)
                y += len(quality_parsed) * (bar_h_q + Emu(45720)) + Emu(274320)

                # Key Initiatives
                q_txt(s2, M, y, SW - 2 * M, Emu(228600),
                      "Key Initiatives Delivered", sz=Pt(11), bold=True, color=Q_PURPLE)
                q_rect(s2, M, y + Emu(228600), Emu(1828800), Emu(18288), Q_PURPLE)
                y += Emu(320040)

                init_box = s2.shapes.add_textbox(M + Emu(45720), y, SW - 2 * M - Emu(91440), Emu(914400))
                tf = init_box.text_frame; tf.word_wrap = True; tf.auto_size = None
                tf.vertical_anchor = MSO_ANCHOR.TOP
                tf.margin_left = Emu(54864); tf.margin_top = Emu(18288)

                for idx_i, item in enumerate(initiatives_parsed):
                    p = tf.paragraphs[0] if idx_i == 0 else tf.add_paragraph()
                    p.space_before = Pt(4); p.space_after = Pt(2)
                    if "\u2192" in item:
                        parts = item.split("\u2192")
                        r1 = p.add_run(); r1.text = f"\u25b8 {parts[0].strip()}"; r1.font.name = 'Mulish'
                        r1.font.size = Pt(9); r1.font.bold = True; r1.font.color.rgb = Q_PURPLE_L
                        r2 = p.add_run(); r2.text = f"  \u2192  {parts[1].strip()}"; r2.font.name = 'Mulish'
                        r2.font.size = Pt(9); r2.font.bold = False; r2.font.color.rgb = Q_GRAY
                    else:
                        r = p.add_run(); r.text = f"\u25b8 {item}"; r.font.name = 'Mulish'
                        r.font.size = Pt(9); r.font.color.rgb = Q_BLACK

                # Thank You footer
                ty = SH - Emu(411480)
                q_rect(s2, 0, ty, SW, Emu(411480), Q_PURPLE)
                q_txt(s2, 0, ty + Emu(45720), SW, Emu(228600),
                      "Thank You", sz=Pt(18), bold=True, color=Q_WHITE, align=PP_ALIGN.CENTER)
                q_txt(s2, 0, ty + Emu(274320), SW, Emu(137160),
                      "Lobby Operations  |  P&T Global Gaming Content",
                      sz=Pt(8), color=RGBColor(0xCE, 0x93, 0xD8), align=PP_ALIGN.CENTER)

                # Save to bytes
                q_buf = io.BytesIO()
                prs_q.save(q_buf)
                q_buf.seek(0)
                q_pptx_bytes = q_buf.getvalue()

                st.success(f"\u2705 Q Report generated! 2 slides — {q_label} ({q_period})")
                st.download_button(
                    label="\U0001f4e5 Download Q Report PPTX",
                    data=q_pptx_bytes,
                    file_name=f"LobbyOps_{q_label.replace(' ', '_')}_Report.pptx",
                    mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",
                    use_container_width=True,
                    key="q_download"
                )
                ...
                st.error(f"Error generating Q Report: {e}")
                import traceback
